...
- **`PageHeader`'s `children` slot overlays the header's bottom border** instead of sitting in flow — it holds `TopProgressBar`, and a bar that comes and goes in flow shifts the whole page by its height.
