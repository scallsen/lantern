...
`StyleGuideLabPage` is one of several `#/dev/*` lab pages that exist to explore a design direction (others: `HomeCardsLabPage`, `TextbookPickerLabPage`, `HomeFlowLabPage`, `SettingsLabPage`, `TextbookFlowLabPage`, `TrackedStatLabPage`, `AccentPolishLabPage`, `CoverRotationLabPage`, `DrillFlipLabPage`, `SecondaryButtonLabPage`, `SegmentColorLabPage`, `ToastLabPage`). None are linked from the dashboard, but all ship in the production bundle at their `#/dev/*` route — **before deleting one of these pages from `main`, copy it onto `archive/design-labs` first.** That branch is a permanent, never-merged, never-deleted home for retired design explorations — `main`'s own history isn't enough once a page's route is removed and later squashed/rebased away. `DeckPickerLabPage` was retired before this convention existed (see settled decision #4) and is not on the archive branch.
...
**Component roster** (`NAV` in `StyleGuideLabPage.jsx` — flip `built: false` → `true` and register in `PAGES` once built). Full call-site detail per component is in `docs/design-system-rebuild-review.md`'s "Component roster — full call-site detail":
