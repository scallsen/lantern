...
| Pinned Shelf | Built — composes Feed Card (`image` mode) + Button (`ghost-muted` remove overlay); Anime Vocab's "Currently studying" |
...
20. A curated pinned set is `PinnedShelf`, not a re-generalized `DataList` — and a reading history gets its own component, not a mode flag on this one.
